...
def fetch_all_inputs(conn):
    """Liefert Liste von (new_txid, change_address)."""
    cur = conn.cursor()
    cur.execute(
        "SELECT DISTINCT change_inputs.new_txid, change_inputs.change_address, replacements.change_vout_index FROM change_inputs INNER JOIN replacements ON replacements.new_txid = change_inputs.new_txid"
    )
    return cur.fetchall()
...
def main():
    ...
    parser.add_argument(
        "--db", required=True, help="Pfad zur SQLite-DB (z.B. mempool.db)"
    )
    parser.add_argument(
        "--out-file",
        required=True,
    ...
    try:
        ...
        print(f"Fehler beim Öffnen der DB {args.db}: {e}", file=sys.stderr)
        sys.exit(1)

    pairs = fetch_all_inputs(conn)
    if not pairs:
        print("Keine Einträge in change_inputs gefunden.", file=sys.stderr)
        sys.exit(0)

    results = []
    success_count = 0

    for new_txid, change_addr, change_vout_index in pairs:
        try:
            tx = fetch_tx_json(new_txid)

            ######################################################################
            # hier die zusatzlichen Daten fetchen, die fur das gnn benotigt werden
            ######################################################################
        ...
        if tx is None:
            print(f"TXID {new_txid} nicht gefunden (404).", file=sys.stderr)
        else:
            success_count += 1
            print(
                f"[{success_count}] TXID {new_txid} erfolgreich abgefragt (Change-Adresse: {change_addr}, Change-Index: {change_vout_index})."
            )
            tx["change_address"] = change_addr
